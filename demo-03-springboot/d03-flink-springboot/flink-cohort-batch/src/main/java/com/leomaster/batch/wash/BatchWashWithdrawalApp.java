...
public class BatchWashWithdrawalApp {

    private final static Logger LOGGER = LoggerFactory.getLogger(BatchWashWithdrawalApp.class);
    ...
    public static void setup(ExecutionEnvironment env, Map<String, Collection<JSONObject>> sourceMap,
        ...
        String sourceQueue = "replay_tb_withdrawal";
        String sinkQueue = "dwd_tb_withdrawal";

        Collection<JSONObject> source = sourceMap.get(sourceQueue);
        if(CollectionUtil.isNullOrEmpty(source)){
            LOGGER.info("BatchWashWithdrawalApp replay_tb_withdrawal is null");
            return;
        }
