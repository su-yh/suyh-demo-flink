...
public class BatchWashUserApp {

    private final static Logger LOGGER = LoggerFactory.getLogger(BatchWashUserApp.class);
    ...
    public static void setup(ExecutionEnvironment env, Map<String, Collection<JSONObject>> sourceMap,
        ...
        String sourceQueue = "replay_tb_user";
        String sinkQueue = "dwd_tb_user";
        Collection<JSONObject> source = sourceMap.get(sourceQueue);
        if(CollectionUtil.isNullOrEmpty(source)){
            LOGGER.info("BatchWashUserApp replay_tb_user is null");
            return;
        }
